...
fams_to_remove <- c('Enterobacteriaceae')

# load correspondng bTAX file with bASV unique identifiers:
...
```{r fig.height=2, fig.width=3, message=FALSE, warning=FALSE, paged.print=FALSE}
ggplot(ELc, aes(x = log(cp,10), y = log(mt,10))) + 
  geom_point() + 
  xlab("log10 mt ASVs") + 
  ylab("log10 cp ASVs") +
...
This certainly passes the sanity check. The estimated slope falls off a bit at the high end, favoring mt counts over cp counts, and we do not know why. It may have something to do with the relative efficacy of the cp versus mt PNA PCR clamps, the per-plant cell organelle genome copy number, or some combination. Either way, we will continue using the sum of cp and mt ASV counts as our reference for the host-derived 16S coverage.
...
CFU_Yrep1 <- ppsim(mod = cfu_m1, nd = nd1, ppstat = 'mean', ppfact = 'herb_dmg', S=100)
CFU_Yrep2 <- ppsim(mod = cfu_m1, nd = nd2, ppstat = 'mean', ppfact = 'herb_dmg', S=100)
...
means_1 <- ggplot() +
  ...
  geom_point(data = MEANS2, aes(y = mn, x = factor(ppfact)), col = "white", shape = 95, size = 8) +
...
EL_sp_cor2 <- dcast(EL_sp_cor, formula = sp_id ~ herb_dmg, value.var = "ln_bac_host", fun.aggregate = mean)
...
NP_sp_cor2 <- dcast(NP_sp_cor, formula = sp_id ~ herb_dmg, value.var = "ln_bac_host", fun.aggregate = mean)
