...
```{r results='asis', message=FALSE, warning=FALSE}
print(xtable(tidy(best_mod, par_type = "non-varying")))
```
...
Now let's output summaries of Family-level (summed) log mean and median CFU per leaf class:

```{r results='asis'}
print(xtable(fam_sum[,grep('mu',names(fam_sum))]))
```

```{r results='asis'}
print(xtable(fam_sum[,grep('med',names(fam_sum))]))
```
...
ggarrange(plotlist = list(fam_lr_raw,fam_plot_raw1))
```

Plots of distributions of medians and means of the log posterior predicted CFU per leaf class:

```{r fig.height=2, fig.width=4.5, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center', fig.cap='Distributions of posterior predicted median log CFU (right) and mean log CFU (middle, right) across 1000 posterior simulation replicates.'}
ggarrange(plotlist = list(fam_plot1_med,fam_plot1,fam_plot2), ncol =3)
```

## Individual bASV results

```{r fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center', fig.cap='bASV-level mean log cfu across 1000 posterior simulations.'}
print(bASV_plot2)
```
