...
run_tot_mods <- function(dd, site, ...){
  ...
  for(m in 1:length(mods)){
    ...
    saveRDS(mods[[m]], file = paste0("../models/all-bacteria/"),'all-bact-mod_',site,"_",names(mods)[m],'.rds'))
  }
...
compare_tot_mods <- function(the_mods, DAT, SET){
  ...
  REP_TEMPLATE <- file.path("./brmsfit_modrep_template_allBact.Rmd")

  # LOAD CFU MODEL
  CFU_MOD_FILE <- file.path("../models/cfu_m1.rds")
  ...
  # this returns an (R*length(bASVs)) by length(samp_id) matrix; takes 1.5 minutes on MacBook Pro 2012
  pplr_by_samp_id <- lapply(sim_dat_l, FUN = leaf_level_pplr, the_mod = best_mod, nsamples = R)
  ...
  for(i in seq_along(sim_dat_l)){
    # need the transpose of the pplr element, since the brms::posterior_predict returns an R by n matrix
    sim_dat_l[[i]] <- cbind(sim_dat_l[[i]], t(pplr_by_samp_id[[i]]))
  }
  ...
  # simulate posterior predicted CFU and log CFU values per sample, where each leaf per rep gets its own joint posterior draw from cfu_mod; takes 30 sec.
  sim_cfu_1 <- pp_cfu_per_leaf_all(sim_dat_l2, cfu_mod, samp_col = 'samples', the_fact = 'sp_id')

  # export for book-keeping:
  write.csv(sim_cfu_1, file = paste0("../models/all-bacteria/","ppcfu_",SET,'.csv'))
  ...
  write.csv(sum_rep, file = paste0("../models/all-bacteria/","sum-all-rep_",SET,'.csv'))
  ...
  write.csv(all_sum, file = paste0("../models/all-bacteria/","sum-all_",SET,'.csv'))
  ...
  rmarkdown::render(input = paste0(REP_TEMPLATE),
                    ...
                    output_dir = "../models/all-bacteria/")
}
...
mEL  <- read.csv(file = file.path("../data/EL_sample_data_final.csv"), row.names = 1)
...
# compare models; generate outputs
